...
cat("\n\nPANNZER annotation by DE\nTRUE if annotated", file = "../Results/Pannzer-by-DE_table.csv", append = T )

kable(missingannotations)
kable(round(prop.table(missingannotations, margin = 1), 4))
...
How many sperm proteins have BLAST annotations?


```{r}
#Cross-Tabulate 

type <- !is.na(merge_by_gene$subject_seq.id) # Logical, TRUE if annotated by BLAST
missingannotations <- table(desc, type)

write.csv(missingannotations, file = "../Results/BLAST-by-DE_table.csv",
            quote = F, col.names = NA, row.names = T )
cat("\n\nBLAST annotation by DE\nTRUE if annotated", file = "../Results/BLAST-by-DE_table.csv", append = T )
...
annotCompByDE <- table(desc, PANNZER_annotations, BLAST_annotations) #Cross tabulate
ftable(annotCompByDE)

write.ftable(ftable(annotCompByDE), file = "../Results/annotationComparisonsByDE.txt",
             quote = F)
